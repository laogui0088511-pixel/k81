...
func StartTransfer(prometheusPort int) error {
	...
	encryptionRpcClient := rpcclient.NewEncryptionRpcClient(client)
	msgTransfer := NewMsgTransfer(chatLogDatabase, msgDatabase, &conversationRpcClient, &groupRpcClient, &encryptionRpcClient)
	return msgTransfer.Start(prometheusPort)
}

func NewMsgTransfer(chatLogDatabase controller.ChatLogDatabase,
	msgDatabase controller.CommonMsgDatabase,
	conversationRpcClient *rpcclient.ConversationRpcClient, groupRpcClient *rpcclient.GroupRpcClient, encryptionRpcClient *rpcclient.EncryptionRpcClient,
) *MsgTransfer {
	return &MsgTransfer{
		persistentCH: NewPersistentConsumerHandler(chatLogDatabase), historyCH: NewOnlineHistoryRedisConsumerHandler(msgDatabase, conversationRpcClient, groupRpcClient, encryptionRpcClient),
