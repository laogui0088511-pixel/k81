...
func NewChatRoute(router gin.IRouter, discov discoveryregistry.SvcDiscoveryRegistry) {
	...
	account.POST("/password/verify_reset_code", chat.VerifyResetPasswordCode) // Verify reset password code
...
func NewAdminRoute(router gin.IRouter, discov discoveryregistry.SvcDiscoveryRegistry) {
	...
	invitationCodeRouter.POST("/set_allow_reset_password", admin.SetInvitationCodeAllowResetPassword) // Set allow reset password
