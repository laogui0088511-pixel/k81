...
// 	}
	...
	for _, register := range invitationRegisters {
		resp.Codes = append(resp.Codes, &admin.InvitationRegister{
			...
			AllowResetPassword: register.AllowResetPassword,
		})
	}
	return resp, nil
}
...
// SetInvitationCodeAllowResetPassword 设置邀请码是否允许用于找回密码
func (o *adminServer) SetInvitationCodeAllowResetPassword(ctx context.Context, req *admin.SetInvitationCodeAllowResetPasswordReq) (*admin.SetInvitationCodeAllowResetPasswordResp, error) {
	defer log.ZDebug(ctx, "return")
	if _, err := mctx.CheckAdmin(ctx); err != nil {
		return nil, err
	}
	if req.Code == "" {
		return nil, errs.ErrArgs.Wrap("code is empty")
	}
	codes, err := o.Database.FindInvitationRegister(ctx, []string{req.Code})
	if err != nil {
		return nil, err
	...
	if err := o.Database.UpdateInvitationRegister(ctx, req.Code, map[string]any{
		"allow_reset_password": req.AllowResetPassword,
	}); err != nil {
		return nil, err
	}
	return &admin.SetInvitationCodeAllowResetPasswordResp{}, nil
}

// VerifyResetPasswordCode 验证找回密码授权码
func (o *adminServer) VerifyResetPasswordCode(ctx context.Context, req *admin.VerifyResetPasswordCodeReq) (*admin.VerifyResetPasswordCodeResp, error) {
	defer log.ZDebug(ctx, "return")
	if req.Code == "" {
		return nil, errs.ErrArgs.Wrap("code is empty")
	}
	...
	if len(codes) == 0 {
		return &admin.VerifyResetPasswordCodeResp{Valid: false}, nil
	}
	// 检查是否已使用且允许用于找回密码
	code := codes[0]
	...
	if code.AllowResetPassword != 1 {
		// 不允许用于找回密码
		return &admin.VerifyResetPasswordCodeResp{Valid: false}, nil
	}
	// 验证手机号是否匹配
	if req.AreaCode != "" && req.PhoneNumber != "" {
		// 使用管理员上下文来调用需要权限的 RPC
		adminCtx := mctx.WithAdminUser(ctx)
		userInfo, err := o.Chat.GetUserFullInfo(adminCtx, code.UsedByUserID)
		if err != nil {
			return nil, err
		}
		if userInfo.AreaCode != req.AreaCode || userInfo.PhoneNumber != req.PhoneNumber {
			return &admin.VerifyResetPasswordCodeResp{Valid: false}, nil
		}
	}
	return &admin.VerifyResetPasswordCodeResp{Valid: true}, nil
}
